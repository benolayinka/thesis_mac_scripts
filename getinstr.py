...
target_triple = args.triple + "-apple-macosx10.13.0"
...
lldb.target = lldb.debugger.CreateTarget(args.binary, target_triple, TARGET_PLATFORM, True, error)
...
if args.symbol:
    breakpoint = lldb.target.BreakpointCreateByName(args.symbol, lldb.target.GetExecutable().GetFilename())
    print breakpoint
...
while True:
    ...
    if not (str(insn.addr).startswith(args.binary)):
    	break
